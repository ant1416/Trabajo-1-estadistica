...
Sexismo benevolente: con un alpha de cronbach de 0,52 su consistencia es baja a moderada, con relacion del ideal de mayor a 0,70, pero considerando que son solo 2 items, esto puede influir y limitar los valores maximos a los que puede llegar la variable. En este sentido, esto sugiere una asociación moderada entre ellos, indicando que ambos capturan aspectos relacionados del constructo. Por tanto, el índice se considera adecuado para fines exploratorios.

sexismo hostil: con un alpha de cronbach de 0,53 aproximadamente, presenta una concistenvia moderada. Si bien este valor se encuentra por debajo del umbral ideal de 0.70, es consistente con lo esperado en escalas compuestas por un número reducido de ítems (en este caso, dos). La correlación promedio entre los ítems (r = 0.36) indica una asociación positiva moderada, lo que sugiere que ambos ítems capturan aspectos relacionados del constructo.

Las subescalas de sexismo benevolente (α = .52) y hostil (α = .53) presentaron una consistencia interna moderada, lo que puede atribuirse al reducido número de ítems y a la heterogeneidad conceptual de las afirmaciones incluidas.

### alpha de crombach: 

dominacion grupal: La escala de dominación grupal presenta una consistencia interna aceptable (α = 0.61), situándose por sobre los valores observados en las escalas de sexismo. Este resultado indica una mayor coherencia entre los ítems que componen el índice.

La correlación promedio entre los ítems (r = 0.44) sugiere una asociación moderada-alta, lo que evidencia que ambos ítems capturan de manera consistente la orientación hacia la jerarquía entre grupos.
...
oposicion a la igualdad: La escala de oposición a la igualdad presenta una alta consistencia interna (α = 0.74), superando el umbral recomendado de 0.70. Este resultado indica que los ítems que componen el índice están fuertemente correlacionados entre sí y capturan de manera coherente el constructo teórico.

meritocracia: La escala de creencias meritocráticas presenta una alta consistencia interna (α = 0.82), superando ampliamente el umbral recomendado de 0.70. Este resultado indica que los ítems que componen el índice están fuertemente correlacionados y capturan de manera coherente el constructo de meritocracia. La correlación promedio entre los ítems (r = 0.70) evidencia una asociación muy alta, lo que sugiere que ambas variables reflejan de forma consistente la creencia de que el éxito social depende de factores individuales como el esfuerzo y las habilidades. La exclusión del ítem c18_13 permitió mejorar significativamente la consistencia interna del índice, lo que evidencia la importancia de evaluar empíricamente la coherencia de los ítems en la construcción de variables latentes
...
Con el objetivo de describir la distribución de los índices construidos, se calcularon estadísticos descriptivos para cada variable continua:

En primer lugar, el sexismo benevolente presenta niveles relativamente altos en la muestra (M = 3.88; Md = 4.00), lo que indica una tendencia general al acuerdo con afirmaciones que refuerzan roles tradicionales de género desde una perspectiva aparentemente positiva.
...
En relación con la dominación grupal, se observan niveles relativamente bajos (M = 2.59; Md = 2.50), lo que indica una tendencia a rechazar afirmaciones que justifican jerarquías explícitas entre grupos sociales.
...
Para evaluar la H1, se realizaron pruebas t de muestras independientes para comparar los niveles de sexismo benevolente y hostil según género. Asimismo, se calcularon correlaciones de Pearson para examinar la relación entre edad y ambas dimensiones del sexismo.
...
No obstante, la magnitud de la correlación es baja, lo que sugiere que, si bien la relación existe, su intensidad es débil. (generaciones).

Tampoco se encontraron diferencias significativas en el sexismo hostil según género (t = 0.55, p = 0.585), observándose medias muy similares entre ambos grupos (M₁ = 3.03; M₂ = 3.02).
